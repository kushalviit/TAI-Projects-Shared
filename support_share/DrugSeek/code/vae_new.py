...
numeric_data = df.values # .values is an attribute of the DataFrame that returns the DataFrame's internal data as a NumPy array.
#scaler = MinMaxScaler()
...
for epoch in range(num_epochs):
    ...
    reconstructed_data = reconstructed_data.cpu() # to convert between GPU and CPU
    ...
    if epoch_correlation > prev_correlation:
       torch.save(vae.state_dict(), 'vae_expression.pth')
    ...
    #    df_embeddings = pd.concat([cell_line_names, df_embeddings], axis=1) #join the cell line names to the embeddings
     #   df_embeddings.to_csv(f'/home/mdicome/CLEmbedding/OmicExpression_embeddings_{epoch}.csv', index=False)
...
plt.legend()
plt.savefig('/data/mikaela/CLEmbedding/vae_loss_curve_Standard.png')
