...
class InteactionsProcessor:
    def __init__(self,file_name):
        self.ppi_table = pd.read_csv(file_name,index_col=None)
    ...
    def remove_unprocessed_genes(self,embeddings_filename):
        processed_genes = pd.read_csv(embeddings_filename,header=None,index_col=None)
    ...
    def mine_ppi(self,embeddings_filename,refined_ppi_file_name):
        ...
        print(f'Length of ppi after removing unprocessed genes  {len(self.ppi_table)}')
    ...
    def save_refined_inteactions(self,refined_ppi_file_name):
        self.ppi_table.to_csv(refined_ppi_file_name,index=False)
...
class siameseDataset(Dataset):
    ...
    def __init__(self,file_name,summary_file_name,usable_genes,non_usable_genes,num_examples):
        self.ppi_table = pd.read_csv(file_name)
    ...
    def __getitem__(self,index):
        ...
        for i in range(self.num_examples):
            ...
            gene0_list.append(self.summary[self.summary.iloc[:,0]==gene0].iloc[0,1:].values.tolist())
            gene1_list.append(self.summary[self.summary.iloc[:,0]==gene1].iloc[0,1:].values.tolist())
            class_list.append(torch.tensor([class_type]))
        
        gene0_list=torch.Tensor(gene0_list)
        gene1_list=torch.Tensor(gene1_list)
        class_list=torch.Tensor(class_list)
        #print(gene0_list.shape)
        #print(class_list.shape)

        return gene0_list, gene1_list, class_list
